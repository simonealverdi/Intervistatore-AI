    ...
    try:
        ...
        if needed_followup:
            try:
                ...
                logger.info(f"Generazione domanda di follow-up per subtopic: {selected_subtopic}")
                try:
                ...
                follow_up_text = None
                # multithread per la generazione della domanda di follow up
                try:
                    if hasattr(session, 'get_notes') and callable(getattr(session, 'get_notes')): # MI SEMBRA INUTILE
                        reflection = session.get_notes() or "Nessuna nota disponibile"
                    else:
                ...
                if follow_up_text:
                    ...
                    session.follow_up_question = follow_up_text
                    session.current_question_is_follow_up_for_subtopic = selected_subtopic # SEMBRA INUTILE
            ...
            if hasattr(state, 'follow_up_question') and session.follow_up_question:
        ...
        try:
            ...
            if question_data and 'text' in question_data:
                ...
                if is_follow_up:
                    ...
                    next_question["Domanda"] = session.follow_up_question
            else:
